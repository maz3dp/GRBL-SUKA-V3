...
void st_wake_up()
{
  ...
  // Initialize step pulse timing from settings. Here to ensure updating after re-writing.
    ...
    _stepper_step_enable(st.step_pulse_time, st.exec_segment->cycles_per_tick);
#endif
}
...
#elif SUKA_V3
        _stepper_step_prescaler(st.exec_segment->prescaler);
#endif        
      #endif
...
#elif SUKA_V3
      _stepper_step_period(st.exec_segment->cycles_per_tick);
#endif
...
{
    _stepper_reset_disable();
    STEP_PORT = (STEP_PORT & ~STEP_MASK) | (step_port_invert_mask & STEP_MASK); //MAZ 
    process_step();  
}
#endif
